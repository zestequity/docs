...
```json
{
  "investors": [
    ...
    {
      "partnerInvestorId": "monark-inv-7342",
...
Each `created` row fires one [`investor.created`](/webhooks/events/investor-created) webhook with the same payload as the row response, plus `kycManagedBy: "partner"` for downstream consumers.
