...
The Zest Public API supports **idempotency** on every write endpoint via the `Idempotency-Key` request header. Idempotency lets you safely retry network failures without risking duplicate side effects.
...
2. If you intended to replay the original — fix the body to match what you sent before, or contact `sara@zestholdco.com` with the `errorId` for support to inspect.
